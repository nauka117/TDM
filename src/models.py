...
def generate_new(unet, noise_scheduler, latent, noise, encoder_hidden_states, prompt_attention_mask, 
    ...
    """
    Generate new latents using TDM approach.
    
    Args:
        unet: The diffusion model (DiT for Open-Sora)
        noise_scheduler: The noise scheduler
        latent: Input latent tensor [B, C, T, H, W]
        noise: Noise tensor [B, C, T, H, W]
        encoder_hidden_states: Text embeddings from text encoder(s)
        prompt_attention_mask: Attention mask for text
        steps: Number of denoising steps
        return_mid: Whether to return intermediate results
        mid_points: Intermediate timesteps
        total_steps: Total number of training timesteps
        add_cfg: CFG configuration dict
    ...
    for ind in range(steps):
        ...
        if use_opensora and isinstance(encoder_hidden_states, list):
        ...
        if add_cfg is not None:
            if use_opensora and isinstance(uncond_prompt_embeds, list):
                uncond_model_kwargs = {
        ...
        # Predict origin using the noise prediction
        ...
        pure_noisy = noise_scheduler.add_noise(latent, noise_pred, T_).to(latent.dtype)
